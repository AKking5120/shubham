export type PricePaper = { name: string; baseRate: number };
export type PriceFinishing = { id: string; name: string; cost: number };

export type PriceCalculatorCategory = {
  name: string;
  papers: PricePaper[];
  quantities: number[];
  finishings: PriceFinishing[];
  qtyUnit: string;
  finishPerQty: boolean;
};

export type PriceCalculatorConfig = Record<string, PriceCalculatorCategory>;

export const DEFAULT_PRICE_CALCULATOR: PriceCalculatorConfig = {
  visiting_cards: {
